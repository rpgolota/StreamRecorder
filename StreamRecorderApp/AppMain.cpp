...
// auto f = co_await KnownFolders::PicturesLibrary().CreateFileAsync(L"TEST.txt");
	...
	m_onlyEyes(false)
{
	...
	// If the only enabled non-research stream is eye collection, only collect eye data no hand data
	if (kEnabledStreamTypes.size() == 1 && std::find(kEnabledStreamTypes.begin(), kEnabledStreamTypes.end(), StreamTypes::EYE) != kEnabledStreamTypes.end()) {
		m_onlyEyes = true;
	}
...
void AppMain::Update()
{
	...
	{		
		...
		// if not only saving eye gaze data
		if (!m_onlyEyes) {
			// Get head transform
...
void AppMain::StopRecording()
{
	...
	m_hethateyeStream.DumpToDisk(m_archiveFolder, m_datetime, m_onlyEyes);
